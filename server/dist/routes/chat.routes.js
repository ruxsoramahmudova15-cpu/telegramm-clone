...
router.post('/create', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        if (isMockMode()) {
            ...
            database_1.mockDb.conversations.set(chatId, chat);
            res.status(201).json({ success: true, chat });
            return;
        }
        // Check if direct chat already exists
        if (type === 'direct') {
            const existingChat = await models_1.Conversation.findOne({
                type: 'direct',
            ...
            if (existingChat) {
                const otherParticipant = existingChat.participants.find((p) => p._id.toString() !== currentUserId);
                res.json({
                    success: true,
                    chat: {
        ...
        // Create new chat
        const newChat = await models_1.Conversation.create({
            type,
            name,
            participants: [currentUserId, participantId],
        ...
        const otherParticipant = newChat.participants.find((p) => p._id.toString() !== currentUserId);
        res.status(201).json({
            success: true,
            chat: {
...
router.get('/my', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        if (isMockMode()) {
            const chats = [];
            for (const chat of database_1.mockDb.conversations.values()) {
        ...
        const chats = await models_1.Conversation.find({
        ...
        const formattedChats = chats.map(chat => {
            const otherParticipant = chat.type === 'direct'
                ? chat.participants.find((p) => p._id.toString() !== currentUserId)
                : null;
            return {
                ...
                lastMessage: chat.lastMessage ? {
                    ...
                    senderId: chat.lastMessage.senderId?._id?.toString(),
...
router.get('/:id', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        const currentUserId = req.user.userId;
        if (isMockMode()) {
            const chat = database_1.mockDb.conversations.get(id);
        ...
        const chat = await models_1.Conversation.findOne({
            _id: id,
            participants: currentUserId
        }).populate('participants', 'displayName username profilePicture isOnline lastSeen');
        if (!chat) {
...
router.get('/:id/messages', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        const chat = await models_1.Conversation.findOne({ _id: id, participants: currentUserId });
        if (!chat) {
